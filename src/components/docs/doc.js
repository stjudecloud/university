    ...
    return (
      <div className="bg-white text-gray-500 antialiased js-focus-visible">
        ...
        >
          <div className="container mt-1 px-9 mx-auto flex items-center justify-center">
            <div>
              ...
              />
              <DocsBody html={html} commits={commits} />
              <DocsFooter previousPage={previousPage} nextPage={nextPage} />
            </div>
          </div>
        </main>
        <footer className="my-10 flex flex-col items-center justify-center">
          <div>
            Built from our{` `}
            <Link to={`https://github.com/stjudecloud/university`}>
              GitHub repo
            </Link>
            {process.env.GATSBY_DOCS_VERSION && (
              <span>
                {` `}at{` `}
                <Link
                  to={`https://github.com/stjudecloud/university/releases/tag/${process.env.GATSBY_DOCS_VERSION}`}
                >
                  {process.env.GATSBY_DOCS_VERSION}
                </Link>
              </span>
