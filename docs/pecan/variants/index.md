...
!!!tip
Updating the filters will **not** update the sunburst selection, but it **will** update the other tabular views (e.g., Variant Prevelance, GenomePaint, and/or ProteinPaint).
!!!
...
!!!tip
The user can navigate the UI using the GenomePaint configs. Only coordinates or gene HUGO names are accepted.
!!!
...
!!!tip
The user can navigate the UI using the ProteinPaint configs. Only coordinates or gene HUGO names are accepted.
!!!

!!!warning
The user is allowed to change reference genomes for this instance, but it will not change across other views, filters, or the membrane.
!!!
...
!!!tip
If there are more than variants at the same coordinate, a user can hover under the bottom variant and find a total which creates the variant table for all variants at that amino acid. Then, they have the ability to select which `pathogenic` or `likely pathogenic` variant details page to navigate to. 
!!!
...
**Figure 10:** (P/LP) Variant Details Page. A user interface that displays pathogenic or likely pathogneic context for recommended variant classfication.
