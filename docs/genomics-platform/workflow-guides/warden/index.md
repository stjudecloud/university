...
| Simple DE analysis          | No advanced statistical analysis, requires only a 1 sample vs 1 sample comparison.                        |
...
| Coverage files (*optional*) | bigWig (`.bw`) files detailing coverage. [(Enabled via `run_coverage` option)](#coverage-results).        |
| FastQC Reports (*optional*) | Quality control analysis by FastQC. [(Enabled via `run_FastQC` option)](#quality-control-results-fastqc). |
